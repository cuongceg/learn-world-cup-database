...
echo  "$($PSQL "SELECT distinct(name) FROM games full join teams on games.winner_id=teams.team_id or games.opponent_id=teams.team_id where name ilike 'Co%' order by name")"
